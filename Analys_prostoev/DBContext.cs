namespace Analys_prostoev
{
    public static class DBContext
    {
        ...
        static public string updateQuery = "UPDATE analysis SET category_one = @categoryOne, category_two = @categoryTwo,category_third = @categoryThird, reason = @reason_new, change_at = @change_at WHERE \"Id\" = @Id";
        static public string changeQuery = "UPDATE analysis SET status = @status, change_at = @change_at WHERE \"Id\" = @id";
        static public string insertQuery = "INSERT INTO analysis (date_start, date_finish, period, region, status, is_manual, created_at, change_at) VALUES (@dateStart, @dateFinish, @period, @region, @status, @is_manual, @created_at, @change_at)";
